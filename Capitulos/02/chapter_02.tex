...
Para exemplos acesse: \url{http://en.wikibooks.org/wiki/LaTeX}
...
\subsection{Equações}
As equações matemáticas são alinhadas à esquerda com recuo de 0,6 cm.  Elas são referidas por Eq. (\ref{eq01}) no meio da frase, ou por Equação (\ref{eq01}) quando usada no início de uma sentença. Os números das equações são numerais arábicos colocados entre parênteses, e alinhados à direita, como mostrado na Eq. (\ref{eq01}).
...
The operation points can be found in Table \ref{tab:OperatingPoints}.

\begin{table}[H]
\caption{Operating Points Values}
\label{tab:OperatingPoints}
\centering
\begin{tabular}{c|cc} \toprule
Parameter & Zeta Mode   & SEPIC Mode \\ \midrule
$I_{La''}$   & 5.634 $A$    & -5.001 $A$  \\ 
$I_{Lb}$   & 7.50 $A$    & -8.18 $A$  \\ 
$V_{Cab}$   & 135.00 $V$    & 131.51 $V$  \\ 
$V_{Cca''}$   & 475.43 $V$    & 331.18 $V$  \\ 
$V_{Ccb}$   & 291.84 $V$    & 511.18 $V$  \\ 
$D$   & 0.5891      & 0.2568    \\ 
\bottomrule
\end{tabular}
\end {table}
